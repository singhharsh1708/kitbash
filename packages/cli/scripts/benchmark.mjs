/**
 * Measured benchmark: one source skill → 7 targets, real token cost per target.
 *
...
// How each target loads a skill. Lazy targets keep only a stub in context until
// the skill is invoked; eager targets carry the whole body every session.
const LOADING = {
  "claude-code": "lazy",
  cursor: "lazy",
  copilot: "eager",
...
function measure(tmp, target, skillName) {
  ...
  switch (target) {
    ...
    case "gemini":
    case "agentsmd": {
      const file = target === "gemini" ? "GEMINI.md" : "AGENTS.md";
...
const skills = [];
try {
  // Every target present so all seven adapters fire.
  for (const d of [".claude", ".cursor", ".clinerules", ".windsurf", ".github"]) mkdirSync(join(tmp, d));
  ...
  // ---- console ----
  console.log("Measured compile cost — one source skill, seven targets (est. ~4 chars/token)\n");
  for (const r of rows) {
  ...
  let md = `# Benchmark: one skill, seven targets
